...
class PasswordlessSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['passwordless_settings']['passwordless_add_css'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Include stylesheet on login page'),
      '#description' => $this->t('If enabled, a stylesheet will be included only on the login page, to style #passwordless-help-link. Clearing the cache might be necessary for any changes to take effect.'),
      '#default_value' => $config->get('passwordless_add_css'),
    ];

    $form['passwordless_settings']['passwordless_show_help'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Show Passwordless help'),
      '#description' => $this->t('Enable help page and provide link to it from the login form.'),
      '#default_value' => $config->get('passwordless_show_help'),
    ];

    $form['passwordless_settings']['passwordless_help_link_text'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Passwordless help-link text'),
      '#description' => $this->t('Text to display in the login form.'),
      '#default_value' => $config->get('passwordless_help_link_text'),
    ];

    $form['passwordless_settings']['passwordless_help_text'] = [
      '#type' => 'text_format',
      '#title' => $this->t('Passwordless help-page text'),
      '#description' => $this->t('Text to display in the help page.'),
    ...
    $form['passwordless_settings']['passwordless_toggle_sent_page'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Enable confirmation page'),
      '#description' => $this->t('Check this box to display the confirmation message on a different page. Uncheck it to display it as a regular Drupal message. @test', [
        '@test' => Link::fromTextAndUrl(
          $this->t('You can also test the confirmation page.'),
          new Url('passwordless.user_login_sent', [], ['attributes' => ['target' => '_blank']])
        )->toString(),
      ]),
    ...
    $form['passwordless_settings']['passwordless_sent_title_text'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Confirmation-page title'),
      '#description' => $this->t('Text to display as the title for the confirmation page.'),
      '#default_value' => $config->get('passwordless_sent_title_text'),
    ];

    $form['passwordless_settings']['passwordless_sent_page_text'] = [
      '#type' => 'text_format',
      '#title' => $this->t('Confirmation-page text'),
      '#description' => $this->t('Text to display as the body for the confirmation page.'),
